...
var bypassMethod = ["signup", "login"];
const bypassHeaderKey = [];

const checkValidationRules = (request, rules) => {
  try {
    const v = new Validator(request, rules);
    const validator = { status: true };

    if (v.fails()) {
      const ValidatorErrors = v.errors.all();
...
export const validateHeaderToken = (authHeader) => {
  try {
    ...
    const decryptedToken = CryptLib.decrypt(
      encryptedToken,
      shaKey,
      GLOBALS.IV
    ).replace(/\s/g, "");

    const verifiedToken = jwt.verify(decryptedToken, GLOBALS.JWT_SECRET);
...
const encString = (data) => {
  try {
    const encryptedData = CryptLib.encrypt(
      JSON.stringify(data),
      shaKey,
      process.env.IV
    );
